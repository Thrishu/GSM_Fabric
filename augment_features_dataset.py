...
def augment_fabric_image(img, augmentation_type):
    ...
    if augmentation_type == 'horizontal_flip':
        ...
        bordered = cv2.copyMakeBorder(img, border, border, border, border, cv2.BORDER_REFLECT)
...
def augment_features_dataset(input_dir="data/feature_extracted_dataset",
                             output_dir="data/augmented_features_dataset",
                             augmentations_per_image=5,
                             preserve_original=True):
    """
    Augment the feature-extracted dataset by:
    1. Creating augmented image versions
    2. Copying features from original images (features are augmentation-invariant)
    """
    ...
    csv_path = input_path / "dataset_with_features.csv"
    df_features = pd.read_csv(csv_path)
    
    # Get feature columns (all except image_name, gsm, source)
    ...
    print("=" * 100)
    print("📸 FEATURE-EXTRACTED DATASET AUGMENTATION")
    print("=" * 100)
    print(f"📊 Original images with features: {len(df_features)}")
    print(f"🔬 Features per image: {len(feature_cols)}")
    print(f"🔄 Augmentations per image: {augmentations_per_image}")
    print(f"📈 Target dataset size: {len(df_features) * (augmentations_per_image + (1 if preserve_original else 0))}")
    print("=" * 100)
    
    augmented_records = []
    
    # Process each image with features
    for idx, row in tqdm(df_features.iterrows(), total=len(df_features), desc="Augmenting"):
        ...
        # Preserve original with its features
        if preserve_original:
            original_output = images_dir / img_name
            cv2.imwrite(str(original_output), img)
            
            record = row.to_dict()
            record['augmentation'] = 'original'
            record['original_image'] = img_name
            augmented_records.append(record)
        
        # Generate augmented versions - COPY FEATURES from original
        ...
        for aug_idx, aug_type in enumerate(selected_augmentations):
            try:
                ...
                # Copy all features from original image
                record = row.to_dict()
                record['image_name'] = aug_name
                record['augmentation'] = aug_type
                record['original_image'] = img_name
    ...
    # Create augmented dataset CSV with features
    ...
    print(f"{'='*100}")
    print(f"📊 Original images: {len(df_features)}")
    print(f"📈 Augmented dataset: {len(df_augmented)} images")
    print(f"🔄 Increase factor: {len(df_augmented) / len(df_features):.1f}x")
    print(f"🔬 Features preserved: {len(feature_cols)}")
    ...
    augmentation_summary = {
        ...
        'increase_factor': len(df_augmented) / len(df_features),
...
def create_stratified_splits(dataset_dir="data/augmented_features_dataset",
    ...
    csv_path = dataset_path / "dataset_with_features.csv"
...
if __name__ == "__main__":
    ...
    # Augment feature-extracted dataset
    df_augmented = augment_features_dataset(
        input_dir="data/feature_extracted_dataset",
        output_dir="data/augmented_features_dataset",
        augmentations_per_image=5,
        preserve_original=True
    )
    ...
    df_splits = create_stratified_splits(
        dataset_dir="data/augmented_features_dataset",
        train_ratio=0.7,
        val_ratio=0.15,
        test_ratio=0.15
    )
    
    print("\n✅ COMPLETE! Dataset ready for training with all features included.")
